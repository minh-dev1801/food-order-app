...
function initData() {
  ...
  const insertMany = db.transaction((meals) => {
    for (const meal of meals) {
      insert.run(meal);
      console.log(`Đã chèn món ăn: ${meal.title}`);
    }
  });

  try {
    insertMany(dummyMeals);
    console.log('Dữ liệu đã được tạo hoặc bản ghi bị trùng bỏ qua.');
  } catch (error) {
    console.log('Dữ liệu khởi tạo lỗi: ', error.message);
  } finally {
    db.close();
